...
function grgw_cc_hitl_rest_push(WP_REST_Request $request) {
    ...
    if (!function_exists('grgw_cc_get_conversation_any')) {
        return grgw_cc_hitl_json_error(
            'grgw_cc_bad_conversation',
            'conversation_id non valido.',
            [ 'conversation_id' => $conversation_id ]
    ...
    $conv = grgw_cc_get_conversation_any($conversation_id);
    if (!$conv) {
        return grgw_cc_hitl_json_error(
            'grgw_cc_bad_conversation',
            'conversation_id non valido.',
    ...
    $was_hitl_active = (int) ($conv['hitl_active'] ?? 0) === 1;
    $should_stop_after_insert = $was_hitl_active && ($meta_type === '' || !in_array($meta_type, $known_hitl_meta, true));
    ...
    if ($should_stop_after_insert) {
        // Se manca il meta poll_stop, assumiamo che un messaggio "finale" (meta vuoto o non riconosciuto)
        // debba chiudere la fase HITL per evitare polling infinito.
        grgw_cc_hitl_set_state_any($conversation_id, false, null);
    }
...
function grgw_cc_hitl_rest_submit(WP_REST_Request $request) {
    ...
    try {
        ...
        $saved_message_id = grgw_cc_hitl_insert_message($conversation_id, 'user', $user_content, $user_meta);
        grgw_cc_hitl_touch_conversation($conversation_id);
