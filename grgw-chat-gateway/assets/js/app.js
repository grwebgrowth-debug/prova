(function(){
  ...
  function updateKeyboardOffset(){
    try{
      ...
      state.ui.keyboardOpen = offset > 80;
    }catch(_){
      root.style.setProperty('--grgw-cc-kb', '0px');
      root.style.setProperty('--grgw-cc-vvh', window.innerHeight + 'px');
      root.style.setProperty('--grgw-cc-vvtop', '0px');
  ...
  function renderMessagesOnly(){
    ...
    const nearBottom = isUserNearBottom(el, 80);
    const shouldStick = shouldAutoScroll(false, nearBottom);
  ...
  function shouldAutoScroll(force = false, nearBottomOverride = null){
    if (force) return true;
    if (state.ui.inputFocused && state.ui.keyboardOpen) return false;
    const wrap = root.querySelector('.grgw-cc-messages');
    const nearBottom = nearBottomOverride !== null ? nearBottomOverride : isUserNearBottom(wrap);
    return nearBottom;
  }
  ...
  async function onSend(){
    ...
    try{
      ...
      // Fetch immediato per sincronizzare eventuali messaggi server-side
      await pollUpdates(true);
  ...
  function mergeServerMessages(newMessages){
    ...
    for (const m of newMessages){
      ...
      let replaced = false;
      for (let i = state.messages.length - 1; i >= 0; i--){
        const local = state.messages[i];
        if (!local || local.id) continue;
        if ((local.role || '') !== (m.role || '')) continue;
        if ((local.content || '') !== (m.content || '')) continue;
  ...
  async function pollUpdates(force = false){
    ...
    if (Array.isArray(data.messages) && data.messages.length){
      const added = mergeServerMessages(data.messages);
      if (added > 0) {
        renderMessagesOnly();
