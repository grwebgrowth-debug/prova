...
// -----------------------------------------------------------------------------
// Public mode scoping (separazione conversazioni guest)
// -----------------------------------------------------------------------------

/**
 * In modalità pubblica, l'utente non loggato deve avere un'identità "device" stabile
 * per evitare che tutti i guest si vedano tra loro (user_id = 0).
 *
 * Soluzione: cookie HttpOnly (grgw_cc_pubk) + thread_id prefissato "pub-{pubk}-...".
...
function grgw_cc_public_get_or_create_key() : string {
    ...
    if (strlen($k) < 16) {
        // token corto ma sufficiente (device key). Non deve essere "segreto" come una password,
        // serve solo come namespace.
        $k = substr(wp_generate_password(32, false, false), 0, 24);

        // Imposta cookie 1 anno, HttpOnly. Secure se HTTPS.
        $secure = is_ssl();
