(function(){
  ...
  function renderMessagesOnly(){
    ...
    const nearBottom = (prevScrollHeight - (prevScrollTop + prevClientHeight)) < 80;
    ...
    requestAnimationFrame(() => {
      ...
      const target = prevScrollTop + Math.max(delta, 0);
      const maxScrollTop = Math.max(0, newScrollHeight - el.clientHeight);
      el.scrollTop = Math.min(target, maxScrollTop);
    });
  }
      ...
      confirmModal(
        ...
        async ()=>{
          try{
            ...
            toast('Errore eliminando: ' + err.message);
          }
        }
      );
      return;
  ...
  async function onSend(){
    ...
    // HITL mode: invio su endpoint dedicato (non passa dall'orchestrator normale)
    if (isHitlActive(conv)){
      ...
      try{
        const type = pendingFile ? (text ? 'mixed' : 'file') : 'text';
        ...
        // Poll immediato per vedere il messaggio appena salvato + eventuali push successivi
        await pollUpdates(true);
      }catch(err){
        toast('Errore HITL: ' + err.message);
      }
    ...
    state.messages.push({ role:'user', content:text, meta:userMeta, created_at: new Date().toISOString() });

    // placeholder assistant
    const placeholderIndex = state.messages.push({ role:'assistant', content:'Sto pensando', meta:null, created_at: new Date().toISOString() }) - 1;
    ...
    try{
      ...
      if (Array.isArray(out.user_attachments) && out.user_attachments.length){
        state.messages[placeholderIndex - 1].meta = { attachments: out.user_attachments };
      }

      const messageIds = out.message_ids || {};
      const userMessageId = Number(messageIds.user || out.user_message_id || 0);
      const assistantMessageId = Number(messageIds.assistant || out.assistant_message_id || 0);

      if (Number.isFinite(userMessageId) && userMessageId > 0){
        state.messages[placeholderIndex - 1].id = userMessageId;
      }

      if (Number.isFinite(assistantMessageId) && assistantMessageId > 0){
        state.messages[placeholderIndex].id = assistantMessageId;
      }

      const lastServerId = Math.max(
        state.poll.lastServerId || 0,
        Number.isFinite(userMessageId) ? userMessageId : 0,
        Number.isFinite(assistantMessageId) ? assistantMessageId : 0
      );
      if (lastServerId > 0) state.poll.lastServerId = lastServerId;

      // replace placeholder with reply
      state.messages[placeholderIndex] = {
        id: (Number.isFinite(assistantMessageId) && assistantMessageId > 0) ? assistantMessageId : undefined,
        role:'assistant',
        content: out.reply || '',
  ...
  async function sendMessage(conversationId, text, provider, model, file){
    if (file){
      ...
      fd.append('files', file); // server prende il primo
  ...
  async function pollUpdates(force = false){
    ...
    const headers = {
      // Il nostro frontend gestisce 304, ma altri client (es. n8n) spesso no.
      // Quindi il backend emette 304 solo se questa preferenza è presente.
      'X-GRGW-PREFER-304': '1',
    };
    ...
    const res = await apiRaw(`/cc/conversations/${conversationId}/updates?after=${encodeURIComponent(String(after))}`, {
      method: 'GET',
      headers,
    });
    ...
    // Aggiorna ETag sempre quando arriva
    ...
    // Schema stabile: { ok, not_modified, messages, conversation }
